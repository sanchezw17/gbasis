...
from gbasis.evals.density import (
    evaluate_density,
    evaluate_rdm_using_evaluated_orbs,
    evaluate_rdm_density,
    evaluate_hole_x2,
...
def test_evaluate_dm_using_evaluated_orbs():
    ...
    density_mat = np.array([[1.0, 2.0], [2.0, 3.0]])
    orb_eval = np.array([[1.0], [2.0]])
    dens = evaluate_rdm_using_evaluated_orbs(density_mat, orb_eval,orb_eval)
...
def test_evaluate_dm_density():
    ...
    dens = evaluate_rdm_density(density, basis, points, transform=transform)
    ...
    dens = evaluate_rdm_density(density, basis, points, point2=points, transform=transform)
    assert np.allclose(np.einsum('ii->i',dens), np.einsum("ij,ik,jk->k", density, evaluate_orbs, evaluate_orbs))
...
def test_evaluate_hole_x2():
    ...
    #points1 = np.random.rand(2000, 3)
    #points2 = np.random.rand(3000, 3)
    points1 = np.random.uniform(0,1,size = (2000,3))
    points2 = np.random.uniform(0,1,size=(3000,3))
